class Transport():
    def __init__(self, coordinates, speed, brand, year, number):
        self.coordinates_ = coordinates
        self.speed_ = speed
        self.brand_ = brand
        self.year_ = year
        self.number_ = number

    @property
    def coordinates(self):
        return self.coordinates_

    @coordinates.setter
    def coordinates(self, value):
        self.coordinates_ = value

    @property
    def speed(self):
        return self.speed_

    @speed.setter
    def speed(self, value):
        self.speed_ = value

    @property
    def brand(self):
        return self.brand_

    @brand.setter
    def brand(self, value):
        self.brand_ = value

    @property
    def year(self):
        return self.year_

    @year.setter
    def year(self, value):
        self.year_ = value

    @property
    def number(self):
        return self.number_

    @number.setter
    def number(self, value):
        self.number_ = value

    def __str__(self):
        return f"Transport: Brand {self.brand}, Year {self.year}, Number {self.number}, Coordinates {self.coordinates}, Speed {self.speed}"

    def isInArea(self, pos_x, pos_y, length, width) -> bool:
        x, y = self.coordinates
        if pos_x <= x <= pos_x + length and pos_y <= y <= pos_y + width:
            return True
        return False


class Passenger():
    def __init__(self):
        self._passengers_capacity = 0
        self._number_of_passengers = 0

    @property
    def passengers_capacity(self):
        return self._passengers_capacity

    @passengers_capacity.setter
    def passengers_capacity(self, value):
        self._passengers_capacity = value

    @property
    def number_of_passengers(self):
        return self._number_of_passengers

    @number_of_passengers.setter
    def number_of_passengers(self, value):
        self._number_of_passengers = value


class Cargo():
    def __init__(self):
        self._carrying = 0

    @property
    def carrying(self):
        return self._carrying

    @carrying.setter
    def carrying(self, value):
        self._carrying = value


class Plane(Transport):
    def __init__(self, coordinat, speed, brand, year, number, height):
        super().__init__(coordinat, speed, brand, year, number)
        self._height = height

    @property
    def height(self):
        return self._height

    @height.setter
    def height(self, value):
        self._height = value


class Auto(Transport):
    def __init__(self, coordinat, speed, brand, year, number):
        super().__init__(coordinat, speed, brand, year, number)


class Ship(Transport):
    def __init__(self, coordinat, speed, brand, year, number, name, port):
        super().__init__(coordinat, speed, brand, year, number)
        self._name = name
        self._port = port

    @property
    def name(self):
        return self._name

    @name.setter
    def name(self, value):
        self._name = value

    @property
    def port(self):
        return self._port

    @port.setter
    def port(self, value):
        self._port = value


class Car(Auto):
    def __init__(self, coordinat, speed, brand, year, number):
        super().__init__(coordinat, speed, brand, year, number)


class Bus(Auto, Passenger):
    def __init__(self, coordinat, speed, brand, year, number):
        super().__init__(coordinat, speed, brand, year, number)


class CargoAuto(Auto, Cargo):
    def __init__(self, coordinat, speed, brand, year, number):
        super().__init__(coordinat, speed, brand, year, number)


class Boat(Ship):
    def __init__(self, coordinat, speed, brand, year, number):
        super().__init__(coordinat, speed, brand, year, number)


class PassengerShip(Ship, Passenger):
    def __init__(self, coordinat, speed, brand, year, number):
        super().__init__(coordinat, speed, brand, year, number)


class CargoShip(Ship, Cargo):
    def __init__(self, coordinat, speed, brand, year, number):
        super().__init__(coordinat, speed, brand, year, number)


class Seaplane(Plane, Ship):
    def __init__(self, coordinat, speed, brand, year, number, height):
        super().__init__(coordinat, speed, brand, year, number, height)
